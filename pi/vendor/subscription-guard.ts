// Request normalization adapted from pi-claude-subscription-connector 1.0.1,
// which vendors @benvargas/pi-claude-code-use 2.0.0. MIT; see LICENSE.subscription.
// Copyright (c) 2026 Ben Vargas and Akshay Patel.
// CCS adaptations: per-registration state and caller-supplied routing predicate.
// Tool aliases, prompt substitutions, history/choice rewriting, configuration,
// and original-tool rendering retain the connector's behavior.
...
// A registration owns its alias maps, including when SDK subagents share this module.
// Vendored imperative helpers retain upstream structure for source comparison.
export default function installSubscriptionGuard(pi: ExtensionAPI, applies: (ctx: ExtensionContext) => boolean) {
// ============================================================================
// Types
// ============================================================================
...
/** PCSC: minimal shape of Pi's theme used by the alias renderCall. */
interface AliasRenderTheme {
...
function registerMcpAliases(pi: ExtensionAPI, opts: { cwd?: string; agentDir?: string } = {}): void {
	...
	const registerAliasTool = (tool: ToolInfo, mcpName: string): void => {
		...
		pi.registerTool({
			...
			// PCSC: display-only. Pi builds the tool-call component during streaming
			// from the alias name and reuses that same component at execution, so
			// without this the mcp__ alias stays on screen for the whole call. This
			// mirrors Pi's own fallback styling, just with the original tool name.
			renderCall: (_args: unknown, theme: AliasRenderTheme) =>
...
// ============================================================================
	...
	pi.on("session_start", async (_event, ctx) => {
		if (applies(ctx)) registerMcpAliases(pi, { cwd: ctx.cwd });
	});

	pi.on("before_agent_start", async (_event, ctx) => {
		const enabled = applies(ctx);
		if (enabled) registerMcpAliases(pi, { cwd: ctx.cwd });
		syncAliasActivation(pi, enabled);
	});
